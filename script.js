...
function createTrend(seed) {
  return Array.from({ length: 18 }, (_, i) => {
    const wave = Math.sin((i + seed) * 0.55) * 10;
    const noise = Math.random() * 7 - 3;
    return Math.max(5, Math.min(98, Math.round(seed + wave + noise)));
  });
}
...
function cardTemplate(metric, idx) {
  ...
  const trend = createTrend(metric.value);
  const spark = polylinePoints(trend.slice(-8), 90, 40);
  const hoverGraph = polylinePoints(trend, 260, 60);
  ...
  return `<article class="card" data-index="${idx}">
      ...
      <div class="value-row">
        ...
        <svg class="sparkline" viewBox="0 0 90 40" preserveAspectRatio="none"><polyline fill="none" stroke="${color}" stroke-width="3" points="${spark}" /></svg>
      </div>
      ...
      <div class="hover-panel">
        ...
        <svg class="hover-chart" viewBox="0 0 260 60" preserveAspectRatio="none"><polyline fill="none" stroke="#ffffff" stroke-width="2.8" points="${hoverGraph}" /></svg>
      </div>
    </article>`;
}

function renderCards() { $('#healthCards').html(metrics.map(cardTemplate).join('')); }

function tick() {
  metrics.forEach((m) => {
    const next = m.value + Math.round(Math.random() * 12 - 6);
    m.value = Math.max(5, Math.min(99, next));
  });
  ...
  $('.card').each(function () {
    ...
    const trend = createTrend(metric.value);
    $(this).find('.sparkline polyline').attr({ points: polylinePoints(trend.slice(-8), 90, 40), stroke: color });
    $(this).find('.hover-chart polyline').attr({ points: polylinePoints(trend, 260, 60) });
  });
}
